...
**If unsure** how to shape invokes, approvals, idempotency, or fields for a `kind`: call **`GET {MERCURY_BASE_URL}/v1/mercury/invoke/guide`** first and follow that Markdown (same host as invoke). Prefer it over memorized patterns.
...
`native_balance` → wallet_address · `erc20_metadata` → chain, token_address · `erc20_balance` → chain, token_address, wallet_address · `erc20_allowance` → chain, token_address, owner_address, spender_address · `contract_read` → chain, contract_address, abi_fragment, function_name, args · `native_transfer` → chain, wallet_id, recipient_address, amount · `erc20_transfer` → chain, wallet_id, token_address, recipient_address, amount · `swap` → chain, wallet_id, from_token, to_token, amount_in, max_slippage_bps, provider_preference (e.g. lifi), idempotency_key.
